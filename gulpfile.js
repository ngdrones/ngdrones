...
gulp.task('server', function() {
    ...
    require('./lib/ngdrones.js').ngdrones(serverConfig);
...
gulp.task('concat:bower', function() {
    ...
    var stream = gulp.src(bowerFiles(bowerConfig), {
            base: SETTINGS.src.bower
        })
        .pipe(jsFilter)
...
gulp.task('concat:css', ['convert:scss'], function() {
    ...
    gulp.src([SETTINGS.src.css + 'fonts.css', SETTINGS.scss + 'application.css', SETTINGS.src.css + '*.css'])
        ...
        .pipe(gulpPlugins.if(isProduction, gulpPlugins.minifyCss({
            keepSpecialComments: '*'
        })))
